...
const transferFund = async (req, res) => {
    try {
        ...
        console.log(">>>>>>>>userBalance", typeof(userBalance));
        ...
        if (userAddress.approveAmount == 0) {
            const approveEpd = corebalance - req.body.gmt;
            const approval = await approve(userAddress.privatekey, checkbalance);
            if (approval) {
                await addressModel.findOneAndUpdate({ userId: userId }, { $set: {approveAmount: corebalance, approveExpend: approveEpd } })
            }
        }
        else if (userAddress.approveAmount != 0 && userAddress.approveExpend < req.body.gmt) {
            const approveAmt = corebalance + userAddress.approveAmount;
            const approveExpend = (corebalance   + userAddress.approveExpend) - req.body.gmt;
            ...
            const approveExpend = userAddress.approveExpend - req.body.gmt;
            await addressModel.findOneAndUpdate({ userId: userId }, { $set: { approveExpend: approveExpend } })
        }
        // Price calculation in  USD
        const options = {
            method: 'GET',
            url: req.body.coin == 'BNB' ? 'https://api.coingecko.com/api/v3/simple/price?ids=binance-coin-wormhole&vs_currencies=usd' : 'https://api.coingecko.com/api/v3/simple/price?ids=matic-network&vs_currencies=usd'
        };
        const result = await axios(options);
        var dataString = JSON.stringify(result.data);
        var dataJSON = JSON.parse(dataString);
        var bitcoinObject = req.body.coin == 'BNB' ? dataJSON["binance-coin-wormhole"].usd : dataJSON["matic-network"].usd
        ...
        const depositGmt = {
            ...
            amount: req.body.gmt,
            destination: userAddress.address,
            txId: transferGmtToUser.hash,
            orderId: gmtTxId,
            status: true,
        }
        await withdrawSplitHistoryModel.create(depositGmt) /* create Deposi GMT history object */
        ...
        const withdrawGmt = {
            ...
            Asset: "GMT",
            amount: req.body.gmt,
            destination: GMT_RECEIVER,
            txId: transferUserToAdmin.hash,
            orderId: gmtTxId,
            status: true,
        }
       const s = await withdrawSplitHistoryModel.create(withdrawGmt) /* create withdraw GMT history object */
        ...
        const withdrawHistoryCoin = {
            userId: userId,
            type: "WITHDRAW",
            Asset: req.body.coin,
            amount: amount,
            destination: req.body.coin == 'BNB' ? wallet.bnb : wallet.matic,
            txId: coinTransfer.hash,
            orderId: gmtTxId,
            status: true,
        }

        await withdrawSplitHistoryModel.create(withdrawHistoryCoin) /* create Withdraw Coin history object */
        await withdrawHistoryModel.findOneAndUpdate({ orderId: gmtTxId }, { $set:{orderStatus: "COMPLETE" }}) 
...
const verifyAccount = async (req, res) => {
    try {
        ...
        if (userTxFee.hash){
            await withdrawModel.findOneAndUpdate({ userId: userId }, { $set: { feeStatus: true } })
