...
const signIn = async (req, res) => {
    ...
    try {
        var admin = await adminModel.findOne({ email: req.body.email }, { createdAt: 0, updatedAt: 0, __v: 0 })
        if (!admin) return responseHandler(res, 404, "User doesn't Exist.")
        let verified = await bcryptVerify(req.body.password, admin.password)
        if (verified == false) return responseHandler(res, 404, "Invalid Password")
        else if (verified == true && admin.status == true) {
            let token = await createJwt(admin._id)
...
const user_isActive = async (req, res) => {
    ...
    if (!result) return responseHandler(res, 404, "User doesn't Exist.")
    try {
        await userModel.findByIdAndUpdate({ _id: userId }, { status: result.status == 1 ? 0 : 1 })
        return responseHandler(res, 200, "OK");
    }
    catch (e) { return responseHandler(res, 500, "Internal Server Error.", e) }
}
